# -*- coding: utf-8 -*-
"""
MCP 服务器完整测试客户端（v1.0）  |  MCP Server Full Test Client (v1.0)
========================================
自动测试 7 项：                    |  Automated 7 tests:
  ① test_api                      |  ① test_api - API connectivity
  ② web_search                    |  ② web_search - Chinese search (verify encoding fix)
  ③ ai_summarize（深度思考）       |  ③ ai_summarize (deep) - AI structured summary + thinking_level
  ④ search_and_summarize 未触发   |  ④ search_and_summarize [NOT TRIGGERED] (large threshold)
  ⑤ search_and_summarize 已触发   |  ⑤ search_and_summarize [TRIGGERED] (small threshold)
  ⑥ search_and_summarize 新参数   |  ⑥ search_and_summarize - New param compatibility
  ⑦ get_tutorial                  |  ⑦ get_tutorial - Tutorial updated

v1.0：④⑤ 显式传 max_refine_rounds=0，功能测试不产生 logs 日志
      （logs 日志只由 text_isolated.py 补搜测试产生，便于检查）
      v1.0: ④⑤ explicitly set max_refine_rounds=0, functional tests do NOT generate logs
"""
...
def call_tool(name, arguments, timeout=TIMEOUT):
    """通过 stdin/stdout 调用 MCP 工具 / Call an MCP tool via stdin/stdout communication."""
    msg = {
...
def extract_text(out):
    """从 MCP 响应中提取文本内容 / Extract text content from MCP response."""
    raw = out
    try:
        resp = json.loads(out)
...
def run_one(name, arguments, check_keywords, display_name, desc=""):
    """
    运行单个测试用例并检查响应中的关键字
    Run a single test case and check expected keywords in response.
    """
...
def main():
    ...
    # Smart summarization [NOT TRIGGERED] - max_refine_rounds=0, no logs
    ...
    # Smart summarization [TRIGGERED] - max_refine_rounds=0, no logs
    results.append(run_one("search_and_summarize",
        {
            "query": "2024年中国GDP", "purpose": "了解经济形势",
    ...
    # ⑦ get_tutorial - 验证新参数已记录 / Verify new params are documented
